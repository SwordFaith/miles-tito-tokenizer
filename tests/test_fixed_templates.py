...
Derived from miles/tests/fast/utils/chat_template_utils/test_fixed_templates.py.
"""
...
from miles_tito_tokenizers import TITOTokenizerType
...
TEMPLATE_DIR = Path(__file__).parent.parent / "src" / "miles_tito_tokenizers" / "templates"
...
def test_subset_caller_falls_through_to_multi_role_row(monkeypatch):
    ...
    monkeypatch.setattr(
        Qwen3TITOTokenizer,
        "SUPPORTED_TEMPLATES",
        (FixedTemplateRow(allowed_roles=frozenset({"tool", "user"}), template="fake.jinja"),),
    )
...
def test_exact_match_wins_over_strict_superset(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "SUPPORTED_TEMPLATES",
        (
            FixedTemplateRow(allowed_roles=frozenset({"tool"}), template="exact.jinja"),
            FixedTemplateRow(allowed_roles=frozenset({"tool", "user"}), template="multi.jinja"),
        ),
    )
    path, kwargs = resolve_fixed_chat_template(TITOTokenizerType.QWEN3, ["tool"])
...
def test_ambiguous_minimal_supersets_raise(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "SUPPORTED_TEMPLATES",
        (
            FixedTemplateRow(allowed_roles=frozenset({"tool", "user"}), template="a.jinja"),
            FixedTemplateRow(allowed_roles=frozenset({"tool", "system"}), template="b.jinja"),
        ),
    )
...
def test_other_tito_model_rows_ignored(monkeypatch):
    ...
    monkeypatch.setattr(
        Qwen35TITOTokenizer,
        "SUPPORTED_TEMPLATES",
        (FixedTemplateRow(allowed_roles=frozenset({"tool"}), template="other.jinja"),),
    )
    with pytest.raises(ValueError, match="SUPPORTED_TEMPLATES"):
        resolve_fixed_chat_template(TITOTokenizerType.QWEN3, ["tool"])
