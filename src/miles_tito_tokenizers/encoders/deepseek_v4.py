...
#   Original path: python/sglang/srt/entrypoints/openai/encoding_dsv4.py
#   Vendored date: 2026-06-19
#   License: Apache License 2.0
#
# The original file header reads:
#   Adapted from the DeepSeek-V4 release reference implementation.
#
# Modifications from upstream:
#   - Removed the unused ``protocol.Tool`` import; this module now uses a
#     package-local Tool compatibility shim.
#   - Added ``is_deepseek_v4()`` and supporting helpers (copied from the
#     original miles bridge) for tokenizer-family detection without SGLang.
...
from miles_tito_tokenizers.encoders._tool import Tool
...
def _build_deepseek_encode_config(kwargs: dict) -> dict:
    ...
    # caller supplies it, and its value is validated by encode_messages (not here).
...
def is_deepseek_v4(tokenizer: Any) -> bool:
    ...
    return _read_model_type(getattr(tokenizer, "name_or_path", "")) == _MODEL_TYPE
...
def sort_tool_results_by_call_order(
    ...
    """Sort tool_result blocks within user messages by tool call order."""
...
def render_messages(
    ...
    **kwargs: Any,
    ...
    """Render *messages* into a DeepSeek V4 prompt string.

    Tools, if given, are injected into the system message.  ``enable_thinking``
    is accepted as a miles-style alias for ``thinking_mode``.
    """
