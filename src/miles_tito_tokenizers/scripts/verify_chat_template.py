...
Derived from miles/scripts/tools/verify_chat_template.py.

Usage examples::

    # Verify a local .jinja template file at raw template-string level
    python -m miles_tito_tokenizers.scripts.verify_chat_template --template path/to/template.jinja

    # Verify a HuggingFace model's chat template at raw template-string level
    python -m miles_tito_tokenizers.scripts.verify_chat_template --model Qwen/Qwen3-0.6B
    ...
    python -m miles_tito_tokenizers.scripts.verify_chat_template --model Qwen/Qwen3-0.6B \\
        --tito-model qwen3 --template miles_tito_tokenizers/templates/qwen3_fixed.jinja
    ...
    python -m miles_tito_tokenizers.scripts.verify_chat_template --model Qwen/Qwen3.5-0.8B --thinking both
"""
...
from miles_tito_tokenizers import TITOTokenizerType, load_hf_chat_template
...
def _load_template_from_model(model_id: str) -> tuple[str, str]:
    """Load the raw HuggingFace chat template for a model."""
    return load_hf_chat_template(model_id), f"HuggingFace: {model_id}"


def _load_tokenizer(model_id: str, chat_template_path: str | None) -> AutoTokenizer:
...
def main() -> int:
    ...
    if use_tito_instance:
        ...
        tokenizer = _load_tokenizer(args.model, template_path)
        if args.template:
